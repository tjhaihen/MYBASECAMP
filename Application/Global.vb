...
Namespace QIS.Web
    ...
    Public NotInheritable Class commonFunction
        ...
        Public Const Key_CacheLoggedOnLinkParamedicID As String = "CacheLoggedOnLinkParamedicID"
        Public Const Key_CacheLoggedOnIsPhysician As String = "CacheLoggedOnIsPhysician"
        ...
        Public Const FORMAT_DATETIME As String = "dd-MMM-yyyy HH:mm"
...
#Region " Set DropDownList "
        Public Shared Sub SetDDL_Table(ByVal ddl As DropDownList, ByVal ddlType As String, ByVal keyField As String, _
            ...
            Select Case ddlType
                ...
                Case "ProjectUserOpenForCustomer"
                    Dim oTbl As New Common.BussinessRules.ProjectUser
                    tblToApply = oTbl.SelectProjectByUserIDOpenForCustomer(keyField.Trim)
                ...
                Case "ClinicByPhysician"
                    Dim oTbl As New Common.BussinessRules.Utility
                    tblToApply = oTbl.GetClinicByPhysician(keyField.Trim)
                Case "Shift"
                    Dim oTbl As New Common.BussinessRules.Utility
                    tblToApply = oTbl.GetShift()
                Case "BudgetYear"
                    Dim oTbl As New Common.BussinessRules.Budget
                    tblToApply = oTbl.GetBudgetYear()
                Case "BudgetMonth"
                    Dim oTbl As New Common.BussinessRules.Budget
                    tblToApply = oTbl.GetBudgetMonth(keyField.Trim)
                Case "StandardCode"
                    Dim oTbl As New Common.BussinessRules.Budget
                    tblToApply = oTbl.GetStandardCode(keyField.Trim)
                Case "BudgetPurchasingYear"
                    Dim oTbl As New Common.BussinessRules.Budget
                    tblToApply = oTbl.GetBudgetPurchasingYear()
                Case "BudgetPurchasingByPeriod"
                    Dim oTbl As New Common.BussinessRules.Budget
                    tblToApply = oTbl.GetBudgetPurchasingByPeriodForDropDown(keyField.Trim)
                Case "DrugInteractionHd"
                    Dim oTbl As New Common.BussinessRules.DrugInteractionHd
                    tblToApply = oTbl.SelectAll()
                Case "Room"
                    Dim oTbl As New Common.BussinessRules.Utility
                ...
                If rgRows.Length > 0 Then
                    ...
                    Do While i <= rgRows.Length
                        Select Case ddlType
                            ...
                            Case "ClinicByPhysician"
                                _strText = Common.ProcessNull.GetString(rgRows(i - 1)("ClinicName"))
                                _strValue = Common.ProcessNull.GetString(rgRows(i - 1)("ClinicID"))
                            Case "Shift"
                                _strText = Common.ProcessNull.GetString(rgRows(i - 1)("ShiftName"))
                                _strValue = Common.ProcessNull.GetString(rgRows(i - 1)("ShiftID"))
                            Case "BudgetYear"
                                _strText = Common.ProcessNull.GetString(rgRows(i - 1)("BudgetYear"))
                                _strValue = Common.ProcessNull.GetString(rgRows(i - 1)("BudgetYear"))
                            Case "BudgetMonth"
                                _strText = Common.ProcessNull.GetString(rgRows(i - 1)("BudgetMonthName"))
                                _strValue = Common.ProcessNull.GetString(rgRows(i - 1)("BudgetMonth"))
                            Case "StandardCode"
                                _strText = Common.ProcessNull.GetString(rgRows(i - 1)("StandardCodeName"))
                                _strValue = Common.ProcessNull.GetString(rgRows(i - 1)("StandardCodeID"))
                            Case "BudgetPurchasingYear"
                                _strText = Common.ProcessNull.GetString(rgRows(i - 1)("BudgetYear"))
                                _strValue = Common.ProcessNull.GetString(rgRows(i - 1)("BudgetYear"))
                            Case "BudgetPurchasingByPeriod"
                                _strText = Common.ProcessNull.GetString(rgRows(i - 1)("BudgetName"))
                                _strValue = Common.ProcessNull.GetString(rgRows(i - 1)("BudgetCode"))
                            Case "DrugInteractionHd"
                                _strText = Common.ProcessNull.GetString(rgRows(i - 1)("GenericName"))
                                _strValue = Common.ProcessNull.GetString(rgRows(i - 1)("DrugInteractionHdID"))
                            Case "Room"
        ...
        Public Shared Sub SetDDL_Period(ByVal ddl As DropDownList)
            ...
            newRow1("PeriodCode") = "ThisWeek"
            newRow1("PeriodName") = "This Week"
            tblYear.Rows.Add(newRow1)
            Dim newRow2 As DataRow = tblYear.NewRow
            newRow2("PeriodCode") = "LastWeek"
            newRow2("PeriodName") = "Last Week"
            tblYear.Rows.Add(newRow2)
            Dim newRow3 As DataRow = tblYear.NewRow
            newRow3("PeriodCode") = "ThisMonth"
            newRow3("PeriodName") = "This Month"
            tblYear.Rows.Add(newRow3)
            Dim newRow4 As DataRow = tblYear.NewRow
            newRow4("PeriodCode") = "LastMonth"
            newRow4("PeriodName") = "Last Month"
...
#End Region
        ...
        Public Shared Function InsertScriptHitungUmur(ByVal calTglLahir As QIS.Web.Calendar, ByVal txtUmurTahun As TextBox, ByVal txtUmurBulan As TextBox, ByVal txtUmurHari As TextBox) As Text.StringBuilder
